...
class JobsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for job in response.xpath('//*[@id="jobList"]/div[1]/ul/li'):
            yield {
                    'jobname': job.xpath('.//@data-positionname').extract_first(),
                    'wage_area': job.xpath('.//div[1]/div[1]/span/text()').extract_first(),
                    'release_date' : job.xpath('.//div[@class="position_name fl"]/span/text()').extract_first(),    
